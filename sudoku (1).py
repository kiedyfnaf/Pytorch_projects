...
# This is a file that has the sudokus, one not solved and one solved for each
dataset = np.loadtxt('sudoku.csv', delimiter=',', skiprows=1, dtype=str)
sudokus = []
solutions = []
num = 50
# Here i put each sudoku to a list, cause it will be useful later
for i in range(num):
    sudoku = dataset[i][0]
    solution = dataset[i][1]

    # Append to the other lists
    sudokus.append(sudoku)
    solutions.append(solution)



# These are constrains for each sudoku, so i have 3 lists each will have 9 inputs corresponding to 9 rows, columns and squares
# The rest just gets the job done
...
for i in range(9):
    for a in range(9):
        ...
        mask.append(help)
# Here i convert each list to a Numpy array and then into tensors cause belive it or not making them a list,
# then an array and then a tensor is actually way faster than making them a tensor right away (for big number of data)
# Also making sure that shapes are correct for the Pytorch
mask = np.array(mask)
...
# Big Class this pretty much is an entire model of the nn
class SudokuSolver(nn.Module):
    ...
    def forward(self, x, device):
        ...
        probabilities = logits.view(81, 9)  # Have to reshape to (81, 9)
        probabilities = self.softmax(probabilities)  # Apply softmax to get probabilities for each
        return probabilities

    # Same thing as above just without softmax
    def forward_no_softmax(self, x):
        ...
        return probabilities
    # This will predict values for each cell and apply constrains to make sure it doesnt input the wrong number
        ...
        with torch.no_grad():
            ...
            for row in range(9):
                for col in range(9):
                    ...
                    elif sudoku[row][col] == 0: # If its empty
    ...
    # Here we choose the cell that it the most confident in its guess
    ...
    # As the name says, it updates the constrains from before, its quite complicated to get right away,
    # but it should be working, this part was really annoying to get right
    ...
    def compute_loss(self, probabilities, solution):
        ...
        loss = criterion(probabilities, solution-1)  # Correct shapes again
        return loss
...
# This loop just runs the code updates the weights, changes the constrains, takes a new sudoku, ...... , and so on, in short makes sure everything that sould be running is running in the correct order
for epoch in range(len(sudokus)):
  ...
  mask = np.array(mask)
  mask = torch.tensor(mask, dtype=torch.float, device =device)
  sudoku = np.array([list(map(int, s)) for s in sudoku])
  solution = np.array([list(map(int, s)) for s in solution])
  sudoku = torch.tensor(sudoku, dtype=torch.float, device =device)
  solution = torch.tensor(solution, dtype=torch.long, device =device)
  solution = solution.view(81)
...
for row in sudoku:
    print(row)
print(loss)
# At this point in time its not working as intended, the code runs but i still have a lot to learn about how to build a network
# and make sure it doesnt get stuck in a local minimum and so i use it as a training ground for my knowledge
# I think it shows perfectly what i can and can't yet do, and ye, i still have a looong way before i will be happy with this one.
