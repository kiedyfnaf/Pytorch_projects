# -*- coding: utf-8 -*-
"""Sudoku-Remastered.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IvXcZPsvfEsTrbmA-CjxGNW33HNwNucC
"""

# This pretty much is the same as the one before, Sudoku(1).py, but here after it guessed a number i let it change it later if it wants to,
# before if it put one i considered it as a new sudoku with this one number, im quite sure this is better idea yet there is still
# a lot missing that makes this not yet good enough for this
import torch
import numpy as np
import os
import math
    ...
    def predict_all_cells(self, sudoku, device):
        with torch.no_grad():
            ...
            for row in range(9):
                for col in range(9):
                    index = row * 9 + col
                    if sudoku_original[row][col] != 0:  # Fixed cell with a given number
...
for epoch in range(len(sudokus)):
  sudoku, sudoku_original = sudokus[epoch], sudokus[epoch]
  ...
  sudoku_original = np.array([list(map(int, s)) for s in sudoku_original])
  solution = np.array([list(map(int, s)) for s in solution])
  sudoku = torch.tensor(sudoku, dtype=torch.float, device =device)
  sudoku_original = torch.tensor(sudoku_original, dtype=torch.float, device =device)
  solution = torch.tensor(solution, dtype=torch.long, device =device)
  solution = solution.view(81)
  sudoku = sudoku.reshape(9,9)
  sudoku_original = sudoku_original.reshape(9,9)
  sudoku_float = sudoku.to(torch.float).view(1,81)
  for i in range(81):
